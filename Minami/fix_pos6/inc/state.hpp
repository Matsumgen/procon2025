...
class State {
    public:
        int x_hosei, y_hosei;  // 相対座標(最初の盤面を基準とした時今の盤面がどれくらいずれているか)
        int rotate_hosei;      // 最初の盤面を基準とした時, 今の盤面がどれくらい回転しているか
        Field f;               // 盤面の情報
        int progress;          // ペアの進捗度(0: 全体回転, 1 ~ f.size: 上端を揃える, f.size + 1 ~ 2 * f.size - 2: 右端を揃える)
        int score;             // 評価
