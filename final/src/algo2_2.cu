...
std::vector<Ope> algo2_2(RawField& field, std::vector<Ope>& opes, std::pair<uint8_t, uint8_t>& offset, uint32_t fsize, MemObj2& mem2) {
  cudaFree(0);
  ...
  bresultOperations[0].assign(opes.begin(), opes.end());
  TasksQueue::start_idx[0] = 0;
  TasksQueue::fieldsiz[0] = opes.size();
  ...
  // fieldsをすべて書き込む
  cudaError_t err = cudaMemcpy(mem2.df, field.data(), field_size * sizeof(uint16_t), cudaMemcpyHostToDevice);
  if (err != cudaSuccess) {
  ...
  while(true) {
    ...
    if(depth >= 22 && paramMode == 0) {
      ...
      /* depth2 = true; */
    } else if(depth >= 40 && paramMode == 1) {
      beam_width = MemObj2::BEAM_WIDTH;
    }
    /* else if(depth >= 40 && paramMode == 1) { */
      /* paramMode = 2; */
      /* beam_search_func = beam_search_kernel_depth2; */
      ...
      /* counter.maxRid = depth2 ? ridsPerField * (1 + ridsPerField) : ridsPerField; */
      /* beam_width = 1 << 14; */
      /* field_len = std::min(field_len, beam_width); */
      /* counter.field_len = field_len; */
    /* } */
    ...
    if(cur.t.score() >= max_score) {
      ...
      push_back_resultOperations(resultOperations[0], cur.t.rid(), fsize, paramMode, depth2, ridsPerField, offset);
      break;
    }
    ...
    for(field_len = 0; field_len < beam_width && !pq.empty(); ++field_len) {
      ...
      push_back_resultOperations(resultOperations[field_len], cur.t.rid(), fsize, paramMode, depth2, ridsPerField, offset);
