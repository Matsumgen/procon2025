...
std::vector<std::array<int, 3>> fdb::getField4(Field f){
  ...
  while(true){
    ...
    if (mdb_data.mv_size == 1) {
      uint8_t value = *((uint8_t*)mdb_data.mv_data);
      if(value == 0)  break;
      std::array<int, 3> ope = decodeOperate(value);
...
std::vector<uint8_t> fdb::encodeField4(Field& f){
  if(f.getSize() != 4){
  ...
  // 最初の8ビットを取得
  uint8_t bi = (g(1,0) << 7) | (g(2,0) << 5) | (g(3,0) << 3) | g(0,1);
  ret.push_back(bi);
...
// 次の8ビットを取得
  ...
  for(uint8_t a : ret)  std::cout << std::bitset<8>(a) << " ";
  std::cout << std::endl;
...
std::array<int, 3> fdb::decodeOperate(uint8_t ope){
  std::array<int, 3> ret = { (ope >> 4) & 3, (ope >> 2) & 3, ope & 3 };
#ifdef DEBUG_FIELD_DB
