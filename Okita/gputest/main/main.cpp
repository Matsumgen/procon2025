...
int main(void) {
  ...
  for(int i = 0; i < SAMPLE_NUM; i++){
    ...
    auto result = algo(start_field.data(), fsize);
  ...
  mean_ope /= SAMPLE_NUM;
  printf("config:\n\tblocksPerGrid:\t\t%d\n\tthreadsPerBlocks:\t%d\n\tBEAM_WIDTH:\t\t%d\n\tFIELDS_PER_THREAD:\t%d\n\tCPU_THREAD_NUM:\t\t%d\n\tQUEUE_SIZE:\t\t%d\n", BLOCKS_PER_GRID, THREADS_PER_BLOCK, BEAM_WIDTH, FIELDS_PER_THREAD, CPU_THREAD_NUM, QUEUE_SIZE);
  printf("time:\n\tmax:\t%f[sec]\n\tmin:\t%f[sec]\n\tmean:\t%f[sec]\n\noperate:\n\tmax:\t%d\n\tmin:\t%d\n\tmean:\t%f\nmean operate per pair:%f\n", max_time, min_time, mean_time, max_ope, min_ope, mean_ope, mean_ope / (fsize * fsize / 2));
