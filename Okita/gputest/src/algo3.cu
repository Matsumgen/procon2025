#define REVIEW_NUM 80
/*
途中まで省いて終了近づいたら完全にする
Nが大きな回転が存在するほど動作が遅くなる

depthとそろう数のグラフを作る
はじめは今ので、最後あたりをdepth2にする？
*/
...
std::vector<uint32_t> algo(uint16_t *start_field, uint32_t fsize) {
  ...
  while(true) {
    ...
    Node cur = pq.top();
    /* printf("start sort depth: %3d, queue: pq.size=%d, max:score=%d/%d, rid=%d, fid=%d\n", depth, (int)pq.size(), (int)cur.t.score >> 6, field_size >> 1, cur.t.rid, cur.t.fid); */
#ifdef REVIEW_NUM
    if(depth < 500) depths[loop][depth] = cur.t.score >> 6;
#endif
    // evalend
    if((cur.t.score >> 6) >= (field_size >> 1)) {
      // 終了処理
