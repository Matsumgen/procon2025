...
void push_back_resultOperations(std::vector<Ope>& resultOperations, const uint32_t& rid, const uint32_t fsize, const uint32_t& paramMode, const bool& depth2, const uint32_t& ridsPerField) {
  if(depth2) {
    resultOperations.push_back(gparamcpu(rid % ridsPerField, fsize, paramMode));
    uint32_t b = rid / ridsPerField;
    if(b != ridsPerField){
      resultOperations.push_back(gparamcpu(b, fsize, paramMode));
    }
  }else{
...
std::vector<Ope> algo2_1(std::vector<RawField>& fields, std::vector<std::vector<Ope>>& opes, uint32_t fsize, MemObj2& mem2) {
  ...
  std::sort(opes.begin(), opes.end(), [](const std::vector<Ope>& a, const std::vector<Ope>& b) { return a.size() < b.size(); });
  ...
  while(true) {
    ...
    if(cur.t.score() >= max_score) {
      ...
      push_back_resultOperations(resultOperations[0], cur.t.rid(), fsize, paramMode, depth2, ridsPerField);
      break;
    }
    ...
    for(field_len = 0; field_len < MemObj2::BEAM_WIDTH && !pq.empty(); ++field_len) {
      ...
      push_back_resultOperations(resultOperations[field_len], cur.t.rid(), fsize, paramMode, depth2, ridsPerField);
